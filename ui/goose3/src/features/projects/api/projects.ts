...
// goose3: project icon discovery used to scan disk via Tauri. Without that
// bridge we surface "no candidates" so the UI gracefully falls back to a
// manual icon picker. See docs/UNSUPPORTED_FEATURES.md.
...
export async function readProjectIcon(_path: string): Promise<ProjectIconData> {
  return Promise.reject(
    new Error("Project icon import is not available in goose3 yet."),
  );
}
